...
func CreatePostHandler(c *gin.Context) {
	//1.获取参数及参数的校验
	//c.shouldbindjson() //validator -> bingding tag
	p := new(models.Post)
	if err := c.ShouldBindJSON(p); err != nil {
		zap.L().Error("c.ShouldBindJSON(p) error", zap.Any("err", err))
...
// 获取帖子详情的处理函数
func GetPostDetialHandler(c *gin.Context) {
	//1.获取参数(从URL中获取帖子的id)
...
func GetPostListHandler2(c *gin.Context) {
	...
	//c.shouldBindJSON() 如果请求中携带的是json格式的数据，才能用这个方法获取到数
